...
# request to the right MCP server.

# 4 - exit_stack is a context manager that will manage the mcp client objects and their sessions and ensures that they 
# are properly closed. In v2, we did not use it because you used the with statement which behind the scenes uses 
# a context manager. Here we may end up using multiple nested with statements 
# since we have multiple servers to connect to. 
# exit_stack allows you to dynamically add the mcp clients and their sessions as you'll see in the code below.

# 5 - connect_to_servers reads the server configuration file and for each single server, it calls the helper method 
# connect_to_server. In this latter method, an MCP client is created and used to launch the server as a sub-process and
...
# This is particularly important in network programming to avoid resource leaks.
